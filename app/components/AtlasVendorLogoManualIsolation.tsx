...
function hideLogoRecords(root: HTMLElement | null) {
  ...
  for (const candidate of candidates) {
    ...
    const row =
      candidate.closest<HTMLElement>("article, [role='listitem']") ||
...
function simplifyManualPreview(root: HTMLElement | null) {
  ...
  let fallback = detail.querySelector<HTMLElement>(".atlas-manual-open-fallback");
  if (!fallback) {
    ...
    if (source) {
      ...
      const openButton = Array.from(root.querySelectorAll<HTMLButtonElement>("button")).find(
    ...
    firstPreview.parentElement?.insertBefore(fallback, firstPreview);
  }
}
...
function vendorRows(payload: any): VendorRow[] {
  const candidates = [
    payload?.vendorRecords,
    payload?.vendors,
    payload?.data?.vendorRecords,
...
async function fetchVendor(propertyId: string, name: string) {
  ...
  if (!response.ok || payload?.ok === false) throw new Error(payload?.error || "Vendor could not be loaded.");
...
async function saveVendorLogo(vendor: VendorRow, propertyId: string, logoDataUrl: string) {
  ...
  const payload = await response.json().catch(() => ({}));
  if (!response.ok || payload?.ok === false) throw new Error(payload?.error || "Vendor logo could not be saved.");
}

function selectedVendorContext(root: HTMLElement) {
  const existingControls = Array.from(root.querySelectorAll<HTMLElement>("button, label")).find((element) => {
  ...
  const controlRow = existingControls.parentElement;
  ...
  if (!name) return null;
  return { controlRow, section, name };
}
...
async function setupVendorLogoIsolation(root: HTMLElement) {
  ...
  for (const child of Array.from(controlRow.children)) {
    ...
    if (child.classList.contains("atlas-vendor-logo-isolated-controls")) continue;
  ...
  if (!controls) {
    ...
    remove.addEventListener("click", async () => {
      ...
      try {
        const vendor = await fetchVendor(propertyId, name);
        if (!vendor) throw new Error("Vendor record was not found.");
        await saveVendorLogo(vendor, propertyId, EMPTY_LOGO);
        patchVendorLogoImages(root, name, EMPTY_LOGO);
  ...
  try {
    ...
    // Legacy logo migration is best-effort and never deletes the old shared record.
  }
}
