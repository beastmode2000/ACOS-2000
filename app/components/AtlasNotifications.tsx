...
export default function AtlasNotifications(props: Props) {
  ...
  const [desktopPanelTop, setDesktopPanelTop] = useState(88);
  ...
  useEffect(() => {
    ...
    const closeOutside = (event: PointerEvent) => {
      ...
      if (
        target &&
        !notificationRootRef.current?.contains(target) &&
  ...
  const toggleNotifications = () => {
    if (!open && !props.isMobile) {
      ...
      if (buttonBounds)
        setDesktopPanelTop(Math.ceil(buttonBounds.bottom + 10));
    }
    setOpen((value) => !value);
  };
  ...
  const alerts = useMemo(() => {
    ...
    if (preferences.inventory)
      props.parts.forEach((part) => {
        ...
        if (
          quantity > minimum &&
          !["Low", "Out", "Order"].includes(String(part.status))
        )
          return;

        next.push({
          id: `part-${part.id}-${quantity}`,
          title: part.name || "Inventory item",
          detail: `Low stock · ${quantity} available`,
          tone: "attention",
          open: props.onOpenParts,
        });
      });
    ...
    if (preferences.inbox)
      props.inboxItems
        .filter((item) =>
          ["New", "Needs Review"].includes(String(item.status)),
        )
        .forEach((item) => {
          next.push({
            id: `inbox-${item.id}-${item.status}`,
            title: item.title || "Inbox item",
            detail:
              item.status === "New"
                ? "New Inbox item"
                : "Waiting for review",
            tone: "info",
            open: () => props.onOpenInbox(String(item.id)),
          });
        });
    ...
    if (preferences.requests)
      props.requests
        .filter((request) =>
          ["New", "Open", "Needs Review"].includes(
        ...
        .forEach((request) => {
          next.push({
            ...
            detail: `Owner request · ${request.status}`,
            tone: "attention",
            open: () => props.onOpenRequest(String(request.id)),
          });
        });
  ...
  async function toggleBrowserAlerts() {
    ...
    try {
      ...
      if (permission !== "granted") {
        setNotificationMessage(
          "Notification permission was not approved.",
        );
        return;
      }
      ...
      if (!response.ok)
        throw new Error("Atlas could not save this device.");
  ...
  return (
    <div
      ...
      className="atlas-notifications-root"
      style={{
        position: "relative",
        marginBottom: 0,
        minWidth: 0,
    ...
    >
      <style>{`
        .atlas-notifications-root > button,
        .atlas-notifications-root > button span,
        .atlas-notifications-panel,
        .atlas-notifications-panel button,
        .atlas-notifications-panel label,
        .atlas-notifications-panel strong,
        .atlas-notifications-panel span {
          color: ${props.colors.navy} !important;
        }
        .atlas-notifications-panel .atlas-notification-detail {
      ...
      >
        <span>
          {props.isMobile ? "Alerts" : "Notifications"}
        </span>
        <span
          style={{
            minWidth: 24,
      ...
      {open && typeof document !== "undefined"
        ? createPortal(
            <div
              ...
              className="atlas-notifications-panel"
              role="dialog"
              aria-modal={props.isMobile}
              aria-label="Atlas notifications"
              style={{
                position: "fixed",
                top: props.isMobile ? 8 : desktopPanelTop,
            ...
            >
              ...
              >
                <strong>Atlas Notifications</strong>
                <button
                  type="button"
                  onClick={markAllRead}
                  style={{
              ...
              >
                <input
                  ...
                  onChange={() => void toggleBrowserAlerts()}
                />
                Phone / browser alerts on this device
              </label>
              ...
              >
                ...
                <select
                  value={notificationPropertyId}
                  onChange={(event) =>
                    void updateNotificationProperty(
              ...
              >
                {(
                  [
                    [
                      "work",
                      "Due, overdue & high-priority work",
                    ],
                    [
                      "inventory",
                      "Low and out-of-stock parts",
                    ],
                ...
                alerts.map((alert) => (
                  <button
                    ...
                    style={{
                      ...
                      borderRadius: 9,
                      background: readIds.includes(alert.id)
                        ? props.colors.panel
                        : "#fff",
                      padding: 10,
                      cursor: "pointer",
                  ...
                  >
                    ...
                    <span
                      className="atlas-notification-detail"
                      style={{
                        display: "block",
                        marginTop: 2,
                        fontSize: 12,
