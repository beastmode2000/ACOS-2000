...
function selectedTeamName() {
  ...
  const heading = header?.querySelector<HTMLElement>("h2, h3, strong");
...
export default function AtlasTeamWorkActions() {
  ...
  const loadTeam = async () => {
    try {
      ...
      if (response.ok && payload?.ok !== false) setMembers(Array.isArray(payload.members) ? payload.members : []);
    } catch {
      // Existing Team UI remains available if this convenience control cannot refresh.
    }
  };
  ...
  if (!host || !selected || selected.active === false || normalized(selected.role) === "master") return null;
  ...
  const viewWork = () => {
    ...
    window.open(`/my-work?memberId=${encodeURIComponent(selected.id)}`, "_blank", "noopener,noreferrer");
  };
  ...
  const copyWorkLink = async () => {
    ...
    setMessage("");
    try {
      const response = await fetch("/api/atlas-work-link", {
        method: "POST",
      ...
      if (!response.ok || !payload?.ok || !payload?.workPath) {
        throw new Error(payload?.error || "Atlas could not create the work link.");
      }
      const link = new URL(String(payload.workPath), window.location.origin).toString();
      try {
      ...
      setMessage(error instanceof Error ? error.message : "Atlas could not create the work link.");
    } finally {
      setBusy(false);
    }
  };
  ...
  return createPortal(
    <div className="atlas-team-work-actions">
      <button type="button" onClick={viewWork}>View Work</button>
      ...
      <style jsx global>{`
        [data-atlas-team-work-actions-host] { display: contents; }
        .atlas-team-work-actions { display: flex; align-items: center; justify-content: flex-end; gap: 6px; flex-wrap: wrap; margin-left: auto; }
        ...
        .atlas-team-work-actions button:first-child { background: #0b3153; border-color: #0b3153; color: #fff; }
        .atlas-team-work-actions button:disabled { opacity: .55; cursor: default; }
        .atlas-team-work-actions span { max-width: 240px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; color: #667788; font-size: 9px; font-weight: 750; }
        @media (max-width: 720px) {
          .atlas-team-work-actions { width: 100%; justify-content: stretch; }
          .atlas-team-work-actions button { flex: 1 1 auto; }
          .atlas-team-work-actions span { width: 100%; max-width: none; }
        }
      `}</style>
    </div>,
    host,
