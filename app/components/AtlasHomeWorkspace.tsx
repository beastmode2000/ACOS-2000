...
export default function AtlasHomeWorkspace({
  ...
  async function syncChore(workOrder: any, meta: HomeRecord) {
    ...
    const savedCore = payload?.record || workOrder;
    const savedMeta = (payload?.meta || meta) as HomeRecord;
    setLocalChores((current) => current.some((item) => String(item?.id || "") === String(savedCore?.id || ""))
      ? current.map((item) => String(item?.id || "") === String(savedCore?.id || "") ? savedCore : item)
      : [savedCore, ...current]);
    ...
    <div style={{ display: "grid", gap: 12, maxWidth: 1500, margin: "0 auto", paddingBottom: isMobile ? 92 : 24 }}>
      ...
      {editingChore && editingMeta ? <div role="dialog" aria-modal="true" onMouseDown={(e)=>{if(e.currentTarget===e.target){setEditingChore(null);setEditingMeta(null);}}} style={{position:"fixed",inset:0,zIndex:13000,background:"rgba(7,27,47,.48)",display:"grid",placeItems:isMobile?"end center":"center",padding:isMobile?0:16}}><section onMouseDown={(e)=>e.stopPropagation()} style={{width:"100%",maxWidth:900,maxHeight:isMobile?"92dvh":"94vh",overflowY:"auto",background:"#FFF",borderRadius:isMobile?"20px 20px 0 0":20,padding:16,boxShadow:"0 24px 80px rgba(7,27,47,.28)"}}>
        ...
        <div style={{display:"grid",gap:10}}>
          ...
          {editingChore.recurring?<div style={{display:"flex",gap:6,flexWrap:"wrap"}}><button type="button" onClick={()=>setEditingChore({...editingChore,recurring:true,recurrenceInterval:1,recurrenceUnit:"Weeks",recurrenceDays:[0,1,2,3,4,5,6]})} style={{...buttonStyle,minHeight:34,padding:"5px 10px",background:String(editingChore.recurrenceUnit||"Weeks")==="Weeks"&&Number(editingChore.recurrenceInterval||1)===1&&normalizedDays(editingChore.recurrenceDays).length===7?"#FFF4CC":"#FFF",borderColor:String(editingChore.recurrenceUnit||"Weeks")==="Weeks"&&Number(editingChore.recurrenceInterval||1)===1&&normalizedDays(editingChore.recurrenceDays).length===7?colors.gold:colors.line}}>Every day</button><button type="button" onClick={()=>setEditingChore({...editingChore,recurring:true,recurrenceInterval:1,recurrenceUnit:"Weeks",recurrenceDays:[1,2,3,4,5]})} style={{...buttonStyle,minHeight:34,padding:"5px 10px"}}>Weekdays</button><button type="button" onClick={()=>setEditingChore({...editingChore,recurring:true,recurrenceInterval:1,recurrenceUnit:"Weeks",recurrenceDays:[0,6]})} style={{...buttonStyle,minHeight:34,padding:"5px 10px"}}>Weekends</button></div>:null}
          {editingChore.recurring && String(editingChore.recurrenceUnit||"Weeks")==="Weeks"?<label style={fieldStyle}>Days<div style={{display:"flex",gap:6,flexWrap:"wrap"}}>{weekdayOptions.map((day)=>{const days=normalizedDays(editingChore.recurrenceDays);const active=days.includes(day.value);return <button key={day.value} type="button" onClick={()=>setEditingChore({...editingChore,recurrenceDays:active?days.filter((value)=>value!==day.value):[...days,day.value]})} style={{...buttonStyle,minHeight:34,padding:"5px 9px",background:active?"#FFF4CC":"#FFF",borderColor:active?colors.gold:colors.line}}>{day.label}</button>;})}</div></label>:null}
