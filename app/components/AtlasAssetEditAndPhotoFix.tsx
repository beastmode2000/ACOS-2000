...
export default function AtlasAssetEditAndPhotoFix() {
  useEffect(() => {
    const handleClick = (event: MouseEvent) => {
      ...
      const drawer = clicked.closest<HTMLElement>(".atlas-asset-reference-drawer, .atlas-asset-drawer");
      if (!drawer) return;

      const nativeEdit = Array.from(drawer.querySelectorAll<HTMLButtonElement>("button")).find((button) => {
        if (button === clicked || button.classList.contains("atlas-asset-inline-action")) return false;
