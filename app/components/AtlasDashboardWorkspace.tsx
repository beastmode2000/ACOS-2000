  ...
  const renderWidgetContent = (id: DashboardWidgetId) => {
    ...
    if (id === "today-upcoming") return (
      <section className="atlas-today-primary" style={cardStyle}>
        ...
        <div style={{ display: "grid", gap: 8, marginTop: 7 }}>
          {dashboardTodayTasks.slice(0, 6).map((task) => { const meta = taskDetails(task.id); return <div key={`today-task-${task.id}`} style={{ display: "grid", gridTemplateColumns: "minmax(0,1fr) auto", gap: 8, border: `1px solid ${colors.line}`, borderRadius: 12, background: "#FFFFFF", padding: 10 }}><button type="button" onClick={() => { openTaskById(task.id); }} style={{ border: 0, background: "transparent", textAlign: "left", padding: 0, cursor: "pointer" }}><strong style={{ display: "block" }}>{task.title}</strong><small style={{ color: "#000000", fontSize: 11, fontWeight: 400, lineHeight: 1.35 }}>Task · {minutesLabel(task.minutes)} · {meta.assignee}</small></button>{meta.assignee !== "Addison" ? <button type="button" onClick={() => updateTaskDetails(task.id, { assignee: "Addison" })} style={{ ...secondaryButtonStyle, width: "auto", minHeight: 30, padding: "4px 8px", fontSize: 11 }}>Give to Addison</button> : <span style={badgeStyle("Scheduled")}>Addison</span>}</div>; })}
        ...
        <div style={{ display: "grid", gap: 8, marginTop: 7 }}>
          {dashboardUpcomingTasks.slice(0, 6).map((task) => <button key={`upcoming-task-${task.id}`} type="button" onClick={() => { openTaskById(task.id); }} style={{ display: "grid", gridTemplateColumns: "minmax(0,1fr) auto", gap: 10, border: `1px solid ${colors.line}`, borderRadius: 12, background: "#FFFFFF", padding: 10, textAlign: "left", cursor: "pointer" }}><span><strong style={{ display: "block" }}>{task.title}</strong><small style={{ color: "#000000", fontSize: 11, fontWeight: 400, lineHeight: 1.35 }}>Task · {formatDate(taskDetails(task.id).dueDate)} · {minutesLabel(task.minutes)}</small></span><span style={badgeStyle(task.priority)}>{task.priority}</span></button>)}
