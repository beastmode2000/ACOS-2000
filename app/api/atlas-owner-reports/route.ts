...
function cleanPropertyId(value: unknown) {
  const propertyId = String(value || "2000")
    .trim()
    .toLowerCase();

  return ["2000", "6855", "3661", "hangar"].includes(
    propertyId,
  )
    ? propertyId
    : "2000";
...
function cleanItems(value: unknown) {
  ...
  return value
    ...
    .map((item, index) => {
      ...
      return {
        id: String(
          row.id || `owner-report-item-${index}`,
        ),
        ...
        person: String(row.person || "").slice(
          0,
          160,
        ),
        department: String(
    ...
    .filter(
      (item) =>
        Boolean(item.date || item.title || item.notes),
    );
}
    ...
    return NextResponse.json(
      {
        ...
        error:
          error instanceof Error
            ? error.message
            : "Owner reports could not be loaded.",
      },
      {
        status: 500,
      },
...
export async function POST(
  ...
  try {
    ...
    const periodStart = cleanDate(
      body.periodStart,
    );

    const periodEnd = cleanDate(
      body.periodEnd,
    );
      ...
      return NextResponse.json(
        {
          ok: false,
          error:
            "Report start and end dates are required.",
        },
        {
          status: 400,
        },
    ...
    const title = String(
      body.title ||
        `Owner Report ${periodStart}–${periodEnd}`,
    )
      .trim()
      .slice(0, 500);

    const status =
      String(body.status || "Draft") === "Final"
        ? "Final"
        : "Draft";
