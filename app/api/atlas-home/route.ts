...
async function seedCookbookOnce(sql: ReturnType<typeof neon>) {
  const markerRows = await sql`
    SELECT id FROM atlas_home_records
    WHERE property_id=${HOME_PROPERTY_ID} AND id=${COOKBOOK_SEED_ID}
    LIMIT 1
  `;
...
async function upsertChoreCalendar(sql: ReturnType<typeof neon>, record: HomeRecord) {
  ...
  const date = String(record.date || "").slice(0, 10) || null;
  const title = `${String(record.emoji || "⭐")} ${record.title}`;
  const person = String(record.person || "Family");
  const points = Number(record.points || 0);
  const notes = [person, points ? `${points} points` : "", String(record.notes || "")].filter(Boolean).join(" · ");
  const repeat = String(record.recurring || "None");

  await sql`
    INSERT INTO atlas_calendar_items
      (id,date,item_date,title,area,category_label,color_id,color_name,all_day,repeat,reminder,notes,linked_type,linked_id,linked_name,completed,source,event_type,property_id)
    VALUES
      (${calendarId},${date},${date},${title},'Family','Chore','home-chore','Blue',true,${repeat},'None',${notes},'Task',${record.id},${record.title},${Boolean(record.completed)},'home-chore','Chore',${HOME_PROPERTY_ID})
    ON CONFLICT (id)
    DO UPDATE SET
      date=EXCLUDED.date,
...
async function deleteChoreCalendar(sql: ReturnType<typeof neon>, id: string) {
  await sql`DELETE FROM atlas_calendar_items WHERE id=${`home-chore-${id}`} AND property_id=${HOME_PROPERTY_ID}`;
}
async function publicShare(sql: ReturnType<typeof neon>, token: string) {
  const rows = await sql`
    SELECT id, person FROM atlas_home_shares
    WHERE property_id=${HOME_PROPERTY_ID} AND token_hash=${tokenHash(token)} AND revoked_at IS NULL
    LIMIT 1
  `;
  return rows[0] as { id?: string; person?: string } | undefined;
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (token) {
      ...
      const person = String(share.person || "Cooper");
      ...
      const calendarRows = await sql`
        SELECT id,item_date,time,end_time,title,area,category_label,notes,event_type,completed,linked_id
      ...
      return NextResponse.json({
        ok:true,
        person,
        records: choreRows.map(r=>r.record),
        calendar: calendarRows.map(row=>({
          id:String(row.id||""),
          date: row.item_date ? String(row.item_date).slice(0,10) : "",
    ...
    const rows = await sql`
      SELECT record FROM atlas_home_records
      WHERE property_id=${HOME_PROPERTY_ID} AND record_type <> 'setting'
      ORDER BY updated_at DESC
    ` as unknown as Array<{ record: HomeRecord }>;
    return NextResponse.json({ ok:true, records:rows.map(r=>r.record) });
  } catch (error) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const body = await request.json() as HomeRecord & { action?: string; person?: string };
    if (body.action === "createShare") {
      if (String(body.propertyId || "") !== HOME_PROPERTY_ID) return NextResponse.json({ok:false,error:"Invalid property."},{status:400});
      const person = String(body.person || "Cooper").trim() || "Cooper";
      const token = randomBytes(32).toString("hex");
      const id = `family-share-${Date.now()}-${randomBytes(4).toString("hex")}`;
      await sql`
        INSERT INTO atlas_home_shares (id,property_id,person,token_hash)
        VALUES (${id},${HOME_PROPERTY_ID},${person},${tokenHash(token)})
      `;
      return NextResponse.json({ok:true,token,person});
    }
    ...
    const allowedTypes: HomeRecordType[] = ["recipe","chore","goal","setting"];
    ...
    const record = { ...body, action:undefined, propertyId:HOME_PROPERTY_ID, id, title, recordType, updatedAt:new Date().toISOString() };
    await sql`
      INSERT INTO atlas_home_records (property_id,id,record_type,record,updated_at)
      VALUES (${HOME_PROPERTY_ID},${id},${recordType},${JSON.stringify(record)}::jsonb,NOW())
      ON CONFLICT (property_id,id)
      DO UPDATE SET record_type=EXCLUDED.record_type,record=EXCLUDED.record,updated_at=NOW()
    `;
    await upsertChoreCalendar(sql, record);
...
export async function PATCH(request: NextRequest) {
  try {
    const sql=getSql();
    await ensureHomeTables(sql);
    const body=await request.json() as {token?:string; choreId?:string};
    ...
    const choreId=String(body.choreId||"").trim();
    const rows=await sql`
      SELECT record FROM atlas_home_records
      WHERE property_id=${HOME_PROPERTY_ID} AND id=${choreId} AND record_type='chore'
      LIMIT 1
    ` as unknown as Array<{record:HomeRecord}>;
    const chore=rows[0]?.record;
    if(!chore) return NextResponse.json({ok:false,error:"Chore not found."},{status:404});
    const person=String(chore.person||"Family");
    if(person!==String(share.person||"Cooper") && person!=="Family") {
      return NextResponse.json({ok:false,error:"This chore is assigned to someone else."},{status:403});
    }
    const recurrence=String(chore.recurring||"None");
    const currentDate=String(chore.date||new Date().toISOString().slice(0,10));
    ...
    const next={
      ...chore,
      completed:recurrence==="None",
      date:recurrence==="None"?currentDate:d.toISOString().slice(0,10),
      completionHistory:[{id:`done-${Date.now()}`,completedAt,points},...(Array.isArray(chore.completionHistory)?chore.completionHistory:[])],
      updatedAt:completedAt,
    };
    await sql`
      UPDATE atlas_home_records
      SET record=${JSON.stringify(next)}::jsonb,updated_at=NOW()
      WHERE property_id=${HOME_PROPERTY_ID} AND id=${choreId}
    `;
    await upsertChoreCalendar(sql,next);
    return NextResponse.json({ok:true,record:next});
  } catch(error) {
    return NextResponse.json({ok:false,error:error instanceof Error?error.message:"Could not update chore."},{status:500});
  }
}
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if(!(await canAccessHome(sql,request))) return privateResponse();
    const rows=await sql`
      SELECT record_type FROM atlas_home_records
      WHERE property_id=${HOME_PROPERTY_ID} AND id=${id}
      LIMIT 1
    ` as unknown as Array<{record_type?:string}>;
    await sql`DELETE FROM atlas_home_records WHERE property_id=${HOME_PROPERTY_ID} AND id=${id}`;
    if(rows[0]?.record_type==="chore") await deleteChoreCalendar(sql,id);
