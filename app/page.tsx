...
export default function AtlasPage() {
  ...
  useEffect(() => { if (ready) window.localStorage.setItem(mapLocalStorageKey, JSON.stringify(mapLabels)); }, [ready, mapLabels]);
  useEffect(() => { if (ready) window.localStorage.setItem(storageKeys.assets, JSON.stringify(assetRecords)); }, [ready, assetRecords]);
  useEffect(() => { if (ready) window.localStorage.setItem(storageKeys.vendors, JSON.stringify(vendorRecords)); }, [ready, vendorRecords]);
  useEffect(() => { if (ready) window.localStorage.setItem(storageKeys.workOrders, JSON.stringify(serviceRecords)); }, [ready, serviceRecords]);
  useEffect(() => { if (ready) window.localStorage.setItem(storageKeys.calendar, JSON.stringify(calendarItems)); }, [ready, calendarItems]);
  useEffect(() => { if (ready) window.localStorage.setItem(storageKeys.parts, JSON.stringify(partRecords)); }, [ready, partRecords]);
  ...
  function locationName(id?: string) { return sortedLocations.find((location) => location.id === id)?.name ?? "General"; }
  function vendorName(id?: string) { return vendorRecords.find((vendor) => vendor.id === id)?.name ?? "No vendor"; }
  function assetName(id?: string) { return assetRecords.find((asset) => asset.id === id)?.name ?? "No asset"; }
  ...
  function stopMapDrag() { draggingLabelRef.current = null; }
  ...
  function handleMapLabelPhotoUpload(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    files.forEach((file) => {
      ...
      reader.onload = () => {
        const photo: UploadedFileRecord = { id: uid("map-photo"), name: file.name, type: file.type || "photo", dataUrl: String(reader.result), createdAt: new Date().toISOString() };
  ...
  function askAtlas() {
    ...
    if (!text) { setAssistantAnswer("Type a question first, then Ask Atlas."); return; }
  ...
  function updateAsset(id: string, changes: Partial<AssetRecord>) { setAssetRecords((current) => current.map((asset) => (asset.id === id ? { ...asset, ...changes } : asset))); }
  function updateWorkOrder(id: string, changes: Partial<ServiceRecord>) { setServiceRecords((current) => current.map((record) => (record.id === id ? { ...record, ...changes } : record))); }
  function updateVendor(id: string, changes: Partial<VendorRecord>) { setVendorRecords((current) => current.map((vendor) => (vendor.id === id ? { ...vendor, ...changes } : vendor))); }
    ...
    <main style={{ ...shellStyle, gridTemplateColumns: isMobile ? "1fr" : "270px 1fr" }}>
      <aside style={{ ...sidebarStyle, position: isMobile ? "static" : "sticky", height: isMobile ? "auto" : "100vh" }}>
        ...
        <nav style={{ ...navStyle, gridTemplateColumns: isMobile ? "repeat(2, minmax(0, 1fr))" : "1fr" }}>
          {screens.map((item) => (
            <button key={item.id} type="button" onClick={() => setScreen(item.id)} style={screen === item.id ? { ...navButtonStyle, ...navButtonActiveStyle } : navButtonStyle}>{item.label}</button>
          ))}
        </nav>
      </aside>
    ...
    return (
      <div style={stackStyle}>
        ...
        <Section title="Priority Work" eyebrow="To Do / Monitor">
          <div style={stackStyle}>
            {sortedWorkOrders.filter((record) => record.status !== "Completed").slice(0, 8).map((record) => (
  ...
  function renderMap() {
    return (
      <Section title="Atlas Property Map" eyebrow="Map" right={<div style={buttonRowStyle}><span style={badgeStyle("Online")}>{mapLabels.length} Labels</span><button type="button" onClick={startNewMapLabel} style={goldButtonStyle}>Add Label</button><button type="button" onClick={resetMapLabels} style={dangerButtonStyle}>Reset Map</button></div>}>
        <div style={{ ...gridTwoStyle, gridTemplateColumns: isMobile ? "1fr" : "1.35fr 0.65fr" }}>
          <div style={{ minWidth: 0 }}>
            <div style={{ ...emptyStateStyle, marginBottom: 12 }}>Full map is visible. Drag labels to adjust placement. Tap a label to edit details.</div>
            <div
              ref={mapRef}
          ...
          <div style={recordCardStyle}>
            ...
            <div style={formGridStyle}>
              ...
              {(mapLabelForm.photos ?? []).length ? (
                <div style={photoGridStyle}>{mapLabelForm.photos.map((photo) => <div key={photo.id} style={photoCardStyle}><img src={photo.dataUrl} alt={photo.name} style={photoImageStyle} /><button type="button" onClick={() => deleteMapPhoto(photo.id)} style={dangerButtonStyle}>Delete</button></div>)}</div>
  ...
  function renderLocations() {
    ...
    return <Section title="Locations" eyebrow="Alphabetized / related assets"><div style={listGridStyle}>{visible.map((location) => { const relatedAssets = assetRecords.filter((asset) => asset.locationId === location.id); return <div key={location.id} style={recordCardStyle}><h3 style={cardTitleStyle}>{location.name}</h3><p style={mutedSmallStyle}>{location.type} · {location.zone}</p><p style={bodyTextStyle}>{location.notes}</p><div style={buttonRowStyle}>{relatedAssets.length ? relatedAssets.map((asset) => <button key={asset.id} type="button" onClick={() => { setSelectedAssetId(asset.id); setScreen("assets"); }} style={chipButtonStyle}>{asset.name}</button>) : <span style={badgeStyle("Monitor")}>No linked assets</span>}</div></div>; })}</div></Section>;
  }
  ...
  function renderAssets() {
    ...
    return (
      <div style={{ ...gridTwoStyle, gridTemplateColumns: isMobile ? "1fr" : "0.9fr 1.1fr" }}>
        <Section title="Assets" eyebrow="Equipment / property records" right={<button type="button" onClick={() => { const record: AssetRecord = { id: uid("asset"), name: "New Asset", locationId: "general", category: "General", status: "Monitor", notes: "", vendorIds: [] }; setAssetRecords((current) => [record, ...current]); setSelectedAssetId(record.id); }} style={goldButtonStyle}>Add Asset</button>}>
  ...
  function renderHistory() {
    return (
      <div style={{ ...gridTwoStyle, gridTemplateColumns: isMobile ? "1fr" : "0.95fr 1.05fr" }}>
        <Section title="Work Orders" eyebrow="To Do / Done" right={<button type="button" onClick={() => { const record: ServiceRecord = { id: uid("wo"), assetId: assetRecords[0]?.id ?? "", date: todayISO(), title: "New Work Order", status: "Open", priority: "Medium", notes: "" }; setServiceRecords((current) => [record, ...current]); setSelectedServiceId(record.id); }} style={goldButtonStyle}>Add Work Order</button>}>
        ...
        <Section title={selectedService.title} eyebrow="Work Order Detail">
          <div style={formGridStyle}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}><label style={labelStyle}>Status<select value={selectedService.status} onChange={(event) => updateWorkOrder(selectedService.id, { status: event.target.value as ServiceStatus })} style={inputStyle}>{["Open", "Scheduled", "Completed", "Monitor"].map((status) => <option key={status} value={status}>{status}</option>)}</select></label><label style={labelStyle}>Priority<select value={selectedService.priority} onChange={(event) => updateWorkOrder(selectedService.id, { priority: event.target.value as WorkOrderPriority })} style={inputStyle}>{["Low", "Medium", "High"].map((priority) => <option key={priority} value={priority}>{priority}</option>)}</select></label></div>
  ...
  function renderVendors() {
    return (
      <div style={{ ...gridTwoStyle, gridTemplateColumns: isMobile ? "1fr" : "0.9fr 1.1fr" }}>
        ...
        <Section title={selectedVendor.name} eyebrow="Vendor Detail"><div style={formGridStyle}><label style={labelStyle}>Name<input value={selectedVendor.name} onChange={(event) => updateVendor(selectedVendor.id, { name: event.target.value })} style={inputStyle} /></label><label style={labelStyle}>Category<input value={selectedVendor.category} onChange={(event) => updateVendor(selectedVendor.id, { category: event.target.value })} style={inputStyle} /></label><label style={labelStyle}>Phone<input value={selectedVendor.phone ?? ""} onChange={(event) => updateVendor(selectedVendor.id, { phone: event.target.value })} style={inputStyle} /></label><label style={labelStyle}>Email<input value={selectedVendor.email ?? ""} onChange={(event) => updateVendor(selectedVendor.id, { email: event.target.value })} style={inputStyle} /></label><label style={labelStyle}>Notes<textarea value={selectedVendor.notes} onChange={(event) => updateVendor(selectedVendor.id, { notes: event.target.value })} style={textareaStyle} /></label></div></Section>
      </div>
    );
  }

  function renderCalendar() { return <Section title="Calendar" eyebrow="Scheduled items"><div style={listGridStyle}>{calendarItems.map((item) => <div key={item.id} style={recordCardStyle}><h3 style={cardTitleStyle}>{item.title}</h3><p style={mutedSmallStyle}>{formatDate(item.date)} · {item.area}</p><span style={badgeStyle(item.status)}>{item.status}</span></div>)}</div></Section>; }
  function renderWeather() { return <Section title="Weather" eyebrow="Property planning"><div style={emptyStateStyle}>Weather planning placeholder. Keep using this tab for rain, wind, lake, exterior stain, irrigation, and dock planning notes.</div></Section>; }
  function renderDocuments() { return <Section title="Documents" eyebrow="Linked records"><div style={listGridStyle}>{defaultDocuments.map((doc) => <div key={doc.id} style={recordCardStyle}><h3 style={cardTitleStyle}>{doc.title}</h3><p style={mutedSmallStyle}>{doc.type} · {doc.area}</p><p style={bodyTextStyle}>{doc.notes}</p>{doc.href ? <a href={doc.href} style={linkStyle}>Open</a> : null}</div>)}</div></Section>; }
  function renderProcedures() { return <Section title="Procedures" eyebrow="Checklists"><div style={listGridStyle}>{defaultProcedures.map((procedure) => <div key={procedure.id} style={recordCardStyle}><h3 style={cardTitleStyle}>{procedure.title}</h3><p style={mutedSmallStyle}>{procedure.area}</p><ol style={{ margin: "10px 0 0", paddingLeft: 20 }}>{procedure.steps.map((step, index) => <li key={`${procedure.id}-${index}`} style={bodyTextStyle}>{step}</li>)}</ol></div>)}</div></Section>; }
  function renderParts() { return <Section title="Parts" eyebrow="Inventory"><div style={listGridStyle}>{partRecords.map((part) => <div key={part.id} style={recordCardStyle}><h3 style={cardTitleStyle}>{part.name}</h3><p style={mutedSmallStyle}>{part.category} · {locationName(part.locationId)}</p><p style={bodyTextStyle}>Qty {part.quantity} / min {part.minQuantity}</p><span style={badgeStyle(part.status)}>{part.status}</span></div>)}</div></Section>; }
  function renderAssistant() { return <Section title="Ask Atlas Property Assistant" eyebrow="Local Atlas records"><div style={formGridStyle}><label style={labelStyle}>Question<textarea value={assistantQuestion} onChange={(event) => setAssistantQuestion(event.target.value)} placeholder="Example: where is the ADU, what is high priority, who handles irrigation..." style={textareaStyle} /></label><button type="button" onClick={askAtlas} style={goldButtonStyle}>Ask Atlas</button><pre style={answerBoxStyle}>{assistantAnswer}</pre></div></Section>; }
}

function Stat({ label, value }: { label: string; value: string }) { return <div style={statCardStyle}><div style={statValueStyle}>{value}</div><div style={statLabelStyle}>{label}</div></div>; }

function Section({ title, eyebrow, right, children }: { title: string; eyebrow: string; right?: React.ReactNode; children: React.ReactNode }) {
  return <section style={sectionStyle}><div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "flex-start", flexWrap: "wrap", marginBottom: 14 }}><div><div style={eyebrowStyle}>{eyebrow}</div><h2 style={sectionTitleStyle}>{title}</h2></div>{right}</div>{children}</section>;
}
...
const mapShellStyle: React.CSSProperties = { position: "relative", width: "100%", overflow: "hidden", borderRadius: 18, border: `1px solid ${colors.line}`, background: "#D7E0EA", userSelect: "none", touchAction: "pan-y" };
const mapImageStyle: React.CSSProperties = { width: "100%", height: "auto", objectFit: "contain", display: "block", pointerEvents: "none" };
...
const mobileMapPinStyle: React.CSSProperties = { fontSize: 10, padding: "5px 7px" };
