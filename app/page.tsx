...
return ( <div> <Header title="Atlas Dashboard" subtitle="Private 2000 estate operations control center." /> <div style={styles.statGrid}> <Stat label="Locations" value={locations.length} /> <Stat label="Assets" value={assets.length} /> <Stat label="Vendors" value={vendors.length} /> <Stat label="Calendar" value={calendar.length} /> </div>
...
return ( <div> <Header title="Property Map" subtitle="The map image stays fixed. Labels are clickable overlays." /> <div style={styles.gridTwo}> <section style={styles.mapCard}> <img src="/atlas-property-map.png" alt="Atlas property map" style={styles.mapImage} />
{labels.map((label) => (
<button
key={label.id}
type="button"
...
return ( <div> <Header title="Locations" subtitle="Clickable and editable estate location records." /> <div style={styles.gridTwo}> <section style={styles.card}>
...
> <strong>{location.name}</strong> <span style={styles.smallMuted}>{location.type}</span> </button>
))} </section>
...
return ( <div> <Header title="Assets" subtitle="Equipment, systems, vehicles, aircraft, appliances, linked vendors, photos, documents, calendar, and service history." /> <div style={styles.gridTwo}> <section style={styles.card}>
<input value={assetSearch} onChange={(e) => setAssetSearch(e.target.value)} placeholder="Search assets..." style={styles.input} /> <div style={styles.scrollList}>
{filteredAssets.map((row) => (
<button key={row.id} type="button" onClick={() => setSelectedAssetId(row.id)} style={row.id === selectedAssetId ? { ...styles.listButton, ...styles.selectedListButton } : styles.listButton}> <strong>{row.name}</strong> <span style={styles.smallMuted}>{row.category} · {row.status}</span> </button>
))} </div> </section>
...
return ( <div> <Header title="Service History" subtitle="Pick the asset, then type or paste service notes, work orders, vendor updates, invoices, repair comments, and linked photos/documents." />
...
return ( <div> <Header title="Vendors" subtitle="Add vendors, edit contacts, link vendors to assets, and track vendor service history." /> <div style={styles.gridTwo}> <section style={styles.card}> <button type="button" onClick={addVendor} style={styles.primaryButton}>+ New Vendor</button>
