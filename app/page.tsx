...
const storageKeys = {
  ...
  assets: ["atlas-asset-records-v3", "atlas-asset-records-v1", "atlas_2000_assets_safe_v1"],
...
function sortByName<T extends { name: string }>(records: T[]) {
  return [...records].sort((a, b) => a.name.localeCompare(b.name));
}

function sortServices(records: ServiceRecord[]) {
  return [...records].sort((a, b) => {
    const aDone = a.status === "Completed" ? 1 : 0;
    const bDone = b.status === "Completed" ? 1 : 0;
    return aDone - bDone || a.date.localeCompare(b.date) || a.title.localeCompare(b.title);
  });
}
...
function Section(props: { title: string; eyebrow?: string; right?: React.ReactNode; children: React.ReactNode }) {
  return (
...
function Stat(props: { label: string; value: string | number; detail: string; onClick?: () => void }) {
  const body = (
    <>
      <div style={{ color: colors.muted, fontSize: 13, fontWeight: 850 }}>{props.label}</div>
      <div style={{ color: colors.navy, fontSize: 34, fontWeight: 950, lineHeight: 1.1, marginTop: 8 }}>{props.value}</div>
      <div style={{ color: colors.muted, fontSize: 13, marginTop: 7 }}>{props.detail}</div>
    </>
  );

  if (props.onClick) {
    return (
      <button type="button" onClick={props.onClick} style={{ ...cardStyle, textAlign: "left", cursor: "pointer" }}>
        {body}
      </button>
    );
  }
...
function Field(props: {
  ...
  return (
    <label style={{ display: "grid", gap: 6 }}>
      ...
      {props.multiline ? (
        <textarea value={props.value} onChange={(event) => props.onChange(event.currentTarget.value)} placeholder={props.placeholder} style={{ ...inputStyle, minHeight: 100, resize: "vertical" }} />
      ) : (
...
export default function AtlasPage() {
  ...
  useEffect(() => {
    ...
    setAssetRecords(storedAssets.length ? sortByName(storedAssets) : fallbackAssets);
    setVendorRecords(storedVendors.length ? sortByName(storedVendors) : fallbackVendors);
    setServiceRecords(storedServices.length ? sortServices(storedServices) : fallbackWorkOrders);
    setProcedureRecords(storedProcedures.length ? storedProcedures.sort((a, b) => a.title.localeCompare(b.title)) : fallbackProcedures);
    setCalendarItems(storedCalendar.length ? storedCalendar.sort((a, b) => a.date.localeCompare(b.date)) : fallbackCalendar);
    setPartRecords(storedParts.length ? sortByName(storedParts) : fallbackParts);
  ...
  useEffect(() => {
    ...
    async function loadAtlasApi() {
      try {
        ...
        setDatabaseStatus(
          `Neon/API loaded: ${apiAssets.length || assetRecords.length} assets, ${apiVendors.length || vendorRecords.length} vendors, ${apiServices.length || serviceRecords.length} work orders.`
        );
      } catch (error) {
        if (!cancelled) {
          setDatabaseStatus("Using saved browser records / fallback records. /api/atlas did not load.");
        }
      }
    }
  ...
  const filteredMapLabels = useMemo(() => {
    ...
    return mapLabels.filter((item) => [item.label, item.category, item.notes].join(" ").toLowerCase().includes(q));
  }, [q, mapLabels]);

  const filteredAssets = useMemo(() => {
    const sorted = sortByName(assetRecords);
    if (!q) return sorted;
  ...
  const filteredVendors = useMemo(() => {
    const sorted = sortByName(vendorRecords);
    if (!q) return sorted;
  ...
  function buildSearchIndex(): SearchResult[] {
    return [
      ...
      ...partRecords.map((item) => ({ id: `part-${item.id}`, type: "Part", title: item.name, subtitle: `${item.category} · Qty ${item.quantity}`, detail: item.notes, screen: "parts" as Screen })),
    ];
  }
  ...
  function addAsset() {
    ...
    setAssetRecords((current) => sortByName([record, ...current]));
    setSelectedAssetId(record.id);
    setScreen("assets");
  }

  function updateAsset(patch: Partial<AssetRecord>) {
    setAssetRecords((current) => sortByName(current.map((item) => (item.id === selectedAsset.id ? normalizeAsset({ ...item, ...patch }) : item))));
  }

  function saveAsset() {
    void postAtlasRecord("assets", selectedAsset);
  }

  function addVendor() {
    const record = normalizeVendor({ id: uid("vendor"), name: "New Vendor", category: "General", notes: "" });
    setVendorRecords((current) => sortByName([record, ...current]));
    setSelectedVendorId(record.id);
    setScreen("vendors");
  }

  function updateVendor(patch: Partial<VendorRecord>) {
    setVendorRecords((current) => sortByName(current.map((item) => (item.id === selectedVendor.id ? normalizeVendor({ ...item, ...patch }) : item))));
  }

  function saveVendor() {
    void postAtlasRecord("vendors", selectedVendor);
  }
  ...
  function addWorkOrder() {
    ...
    setServiceRecords((current) => sortServices([record, ...current]));
    setSelectedServiceId(record.id);
    setScreen("history");
  }

  function updateWorkOrder(patch: Partial<ServiceRecord>) {
    setServiceRecords((current) => sortServices(current.map((item) => (item.id === selectedService.id ? normalizeService({ ...item, ...patch }) : item))));
  }

  function saveWorkOrder() {
    void postAtlasRecord("work_orders", selectedService);
  }

  function addCalendarItem() {
    const record = normalizeCalendar({ id: uid("cal"), title: "New Calendar Item", date: todayISO(), area: "2000", status: "Scheduled" });
    setCalendarItems((current) => [record, ...current].sort((a, b) => a.date.localeCompare(b.date)));
    setSelectedCalendarId(record.id);
    setScreen("calendar");
  }

  function updateCalendarItem(patch: Partial<CalendarItem>) {
    setCalendarItems((current) => current.map((item) => (item.id === selectedCalendar.id ? normalizeCalendar({ ...item, ...patch }) : item)).sort((a, b) => a.date.localeCompare(b.date)));
  }

  function saveCalendarItem() {
    void postAtlasRecord("calendar", selectedCalendar);
  }

  function updateProcedure(patch: Partial<ProcedureRecord>) {
    setProcedureRecords((current) => current.map((item) => (item.id === selectedProcedure.id ? normalizeProcedure({ ...item, ...patch }) : item)).sort((a, b) => a.title.localeCompare(b.title)));
  }

  function saveProcedure() {
    void postAtlasRecord("procedures", selectedProcedure);
  }

  function updatePart(patch: Partial<PartRecord>) {
    setPartRecords((current) => sortByName(current.map((item) => (item.id === selectedPart.id ? normalizePart({ ...item, ...patch }) : item))));
  }
  ...
  function addMapLabel() {
    ...
    setMapLabels((current) => [...current, record]);
  ...
  function askAtlas() {
    ...
    if (text.includes("map")) {
      setAssistantAnswer(`The map is locked to /atlas-property-map.png and currently has ${mapLabels.length} movable labels. Use Reset Map to restore the saved label layout.`);
      return;
    }

    if (text.includes("asset") || text.includes("equipment")) {
      setAssistantAnswer(`Atlas currently has ${assetRecords.length} asset records loaded. Search or open Assets to review/edit them.`);
      return;
    }

    if (text.includes("vendor")) {
      setAssistantAnswer(`Atlas currently has ${vendorRecords.length} vendors loaded. Search or open Vendors to review/edit them.`);
      return;
    }
    ...
    if (text.includes("database") || text.includes("neon")) {
      setAssistantAnswer(databaseStatus);
      return;
    }

    setAssistantAnswer(`Loaded now: ${assetRecords.length} assets, ${vendorRecords.length} vendors, ${serviceRecords.length} work orders, ${procedureRecords.length} procedures, ${calendarItems.length} calendar items, and ${partRecords.length} parts.`);
  }
    ...
    return (
      <div style={stackStyle}>
        <div style={statGridStyle}>
          <Stat label="Assets" value={assetRecords.length} detail={`${monitoredAssets.length} monitor/offline`} onClick={() => setScreen("assets")} />
          <Stat label="Vendors" value={vendorRecords.length} detail="Service directory" onClick={() => setScreen("vendors")} />
          <Stat label="Open WOs" value={openWorkOrders.length} detail={`${highPriority.length} high priority`} onClick={() => setScreen("history")} />
          <Stat label="Map Labels" value={mapLabels.length} detail="Locked original map" onClick={() => setScreen("map")} />
        </div>

        <Section title="Atlas Status" eyebrow="Restore">
          <div style={noticeStyle}>
            <strong>{databaseStatus}</strong>
            <p style={mutedSmallStyle}>This page loads from /api/atlas first, then saved browser records, then fallback records. The map is forced to /atlas-property-map.png.</p>
          </div>
        </Section>

        <Section title="Priority Work" eyebrow="To Do / Monitor" right={<button type="button" onClick={addWorkOrder} style={goldButtonStyle}>Add Work Order</button>}>
          <div style={listStyle}>
            {openWorkOrders.slice(0, 8).map((record) => (
  ...
  function renderLocations() {
    return (
      <Section title="Locations" eyebrow="2000 Areas">
          ...
          {filteredLocations.map((location) => (
            <button key={location.id} type="button" onClick={() => { setQuery(location.name); }} style={rowButtonStyle}>
              <div>
                ...
                <p style={mutedSmallStyle}>{location.notes}</p>
              </div>
  ...
  function renderAssets() {
    return (
      <Section title="Assets" eyebrow="Equipment / Records" right={<button type="button" onClick={addAsset} style={goldButtonStyle}>Add Asset</button>}>
        <div style={{ ...gridTwoStyle, gridTemplateColumns: isMobile ? "1fr" : "0.9fr 1.1fr" }}>
          ...
          <div style={detailCardStyle}>
            ...
            <div style={buttonRowStyle}>
              <button type="button" onClick={saveAsset} style={goldButtonStyle}>Save Asset</button>
                ...
                <div style={photoGridStyle}>
                  {selectedAssetPhotos.map((photo) => (
                    <div key={photo.id} style={photoCardStyle}>
                      ...
                      <p style={mutedSmallStyle}>{photo.createdAt ? formatDate(photo.createdAt.slice(0, 10)) : "Photo record"}</p>
                    </div>
                  ))}
                </div>
              ) : (
  ...
  function renderVendors() {
    return (
      <Section title="Vendors" eyebrow="Service Directory" right={<button type="button" onClick={addVendor} style={goldButtonStyle}>Add Vendor</button>}>
        <div style={{ ...gridTwoStyle, gridTemplateColumns: isMobile ? "1fr" : "0.9fr 1.1fr" }}>
          <div style={listStyle}>
          ...
          <div style={detailCardStyle}>
            ...
            <button type="button" onClick={saveVendor} style={goldButtonStyle}>Save Vendor</button>
          </div>
        </div>
      </Section>
    );
  ...
  function renderWorkOrders() {
    return (
      <Section title="Work Orders" eyebrow="Service History" right={<button type="button" onClick={addWorkOrder} style={goldButtonStyle}>Add Work Order</button>}>
        <div style={{ ...gridTwoStyle, gridTemplateColumns: isMobile ? "1fr" : "0.95fr 1.05fr" }}>
          ...
          <div style={detailCardStyle}>
            ...
            <div style={formGridStyle}>
              ...
              <label style={{ display: "grid", gap: 6 }}>
                ...
                <select value={selectedService.assetId} onChange={(event) => updateWorkOrder({ assetId: event.currentTarget.value })} style={inputStyle}>
                  ...
                  {sortByName(assetRecords).map((asset) => <option key={asset.id} value={asset.id}>{asset.name}</option>)}
                </select>
              </label>
              <label style={{ display: "grid", gap: 6 }}>
                ...
                <select value={selectedService.vendorId ?? ""} onChange={(event) => updateWorkOrder({ vendorId: event.currentTarget.value })} style={inputStyle}>
                  ...
                  {sortByName(vendorRecords).map((vendor) => <option key={vendor.id} value={vendor.id}>{vendor.name}</option>)}
                </select>
              </label>
            ...
            <button type="button" onClick={saveWorkOrder} style={goldButtonStyle}>Save Work Order</button>
          </div>
        </div>
      </Section>
    );
  ...
  function renderCalendar() {
    return (
      <Section title="Calendar" eyebrow="Scheduled Items" right={<button type="button" onClick={addCalendarItem} style={goldButtonStyle}>Add Calendar Item</button>}>
        <div style={{ ...gridTwoStyle, gridTemplateColumns: isMobile ? "1fr" : "0.95fr 1.05fr" }}>
          <div style={listStyle}>
          ...
          <div style={detailCardStyle}>
            ...
            <button type="button" onClick={saveCalendarItem} style={goldButtonStyle}>Save Calendar Item</button>
          </div>
        </div>
      </Section>
    );
  ...
  function renderDocuments() {
    return (
      <Section title="Documents / Photos" eyebrow="Records">
        <div style={recordGridStyle}>
          {documents.map((document) => (
            <div key={document.id} style={cardStyle}>
              <div style={eyebrowStyle}>{document.type}</div>
              <h3 style={smallTitleStyle}>{document.title}</h3>
              <p style={mutedSmallStyle}>{document.area}</p>
          ...
          {photos.map((photo) => (
            <div key={photo.id} style={cardStyle}>
              <div style={eyebrowStyle}>Photo</div>
              <h3 style={smallTitleStyle}>{photo.name}</h3>
      ...
      <Section title="Procedures" eyebrow="Checklists">
        <div style={{ ...gridTwoStyle, gridTemplateColumns: isMobile ? "1fr" : "0.95fr 1.05fr" }}>
          ...
          <div style={detailCardStyle}>
            ...
            <button type="button" onClick={saveProcedure} style={goldButtonStyle}>Save Procedure</button>
          </div>
        </div>
      </Section>
    );
  ...
  function renderWeather() {
    return (
      <Section title="Weather" eyebrow="Property Watch">
        <div style={noticeStyle}>
          Weather screen restored as a placeholder. No weather API or auth was changed.
        </div>
      </Section>
    );
  }
  ...
  function renderAssistant() {
    return (
      <Section title="Ask Atlas" eyebrow="Local Property Assistant">
        <div style={stackStyle}>
          <textarea
            value={assistantQuestion}
            onChange={(event) => setAssistantQuestion(event.currentTarget.value)}
            placeholder="Ask about assets, vendors, work orders, map, database, or records..."
  ...
  return (
    <main style={{ minHeight: "100vh", background: colors.bg, color: colors.text }}>
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "260px 1fr", minHeight: "100vh" }}>
        <aside style={{ background: colors.navy, color: "#FFFFFF", padding: isMobile ? 14 : 20, position: isMobile ? "static" : "sticky", top: 0, height: isMobile ? "auto" : "100vh", overflow: "auto" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 18 }}>
            <div style={{ width: 42, height: 42, borderRadius: 14, background: colors.gold, color: colors.navy, display: "grid", placeItems: "center", fontWeight: 950, fontSize: 22 }}>A</div>
            <div>
              <div style={{ fontWeight: 950, fontSize: 20, letterSpacing: 1 }}>ATLAS</div>
              <div style={{ color: "#B7C7D8", fontSize: 12, fontWeight: 800 }}>2000 Estate System</div>
            </div>
          </div>
          ...
          <nav style={{ display: "grid", gap: 8, gridTemplateColumns: isMobile ? "repeat(2, minmax(0, 1fr))" : "1fr" }}>
            {screens.map((item) => (
              <button
                ...
                style={{
                  border: `1px solid ${screen === item.id ? colors.gold : "rgba(255,255,255,0.12)"}`,
        ...
        <section style={{ minWidth: 0 }}>
          <header style={{ background: colors.card, borderBottom: `1px solid ${colors.line}`, padding: isMobile ? 14 : 20, position: "sticky", top: 0, zIndex: 20 }}>
            <div style={{ display: "flex", flexDirection: isMobile ? "column" : "row", gap: 12, justifyContent: "space-between", alignItems: isMobile ? "stretch" : "center" }}>
              <div>
                ...
                <h1 style={{ margin: 0, color: colors.navy, fontSize: isMobile ? 24 : 30, fontWeight: 950 }}>{screens.find((item) => item.id === screen)?.label}</h1>
              </div>

              <div style={{ position: "relative", width: isMobile ? "100%" : 430, maxWidth: "100%" }}>
                <input
                  value={query}
...
const cardStyle: React.CSSProperties = {
  ...
  borderRadius: 20,
  padding: 16,
  boxShadow: "0 10px 24px rgba(11, 30, 51, 0.05)",
};
